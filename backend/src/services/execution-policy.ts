...
export type PolymarketStatus = "UNKNOWN" | "UP" | "HAS_ISSUES" | "UNDER_MAINTENANCE";
...
async function checkStatus() {
  try {
    ...
    if (newStatus && newStatus !== currentStatus && ["UP", "HAS_ISSUES", "UNDER_MAINTENANCE"].includes(newStatus)) {
      logger.info({ old: currentStatus, new: newStatus }, "Polymarket status changed");
...
export const executionPolicy = {
  ...
  canExecuteStopLoss: () => currentStatus === "UP" || currentStatus === "HAS_ISSUES"
};
